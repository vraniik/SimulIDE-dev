...
static bool ScriptArrayTemplateCallback( asITypeInfo *ti, bool &dontGarbageCollect )
{
	...
	if( (typeId & asTYPEID_MASK_OBJECT) && !(typeId & asTYPEID_OBJHANDLE) )
	{
        ...
        if( !(flags & asOBJ_GC) ) dontGarbageCollect = true;
	}
...
void RegisterScriptArray( asIScriptEngine *engine, bool defaultArray )
{
    ...
    else RegisterScriptArray_Generic(engine);
...
CScriptArray &CScriptArray::operator=(const CScriptArray &other)
{
    ...
    {
        Resize(other.buffer->numElements); // Make sure the arrays are of the same size
        CopyBuffer(buffer, other.buffer); // Copy the value of each element
	}
	return *this;
}
...
CScriptArray::CScriptArray(asITypeInfo *ti, void *buf)
{
    ...
    else elementSize = engine->GetSizeOfPrimitiveType(subTypeId);
...
CScriptArray::CScriptArray(asUINT length, void *defVal, asITypeInfo *ti)
{
    ...
    for( asUINT n = 0; n < GetSize(); n++ ) SetValue(n, defVal);
}
...
{
    if( maxElements <= buffer->maxElements ) return;
    if( !CheckMaxSize(maxElements) ) return;
    ...
    userFree( buffer ); // Release the old buffer
	buffer = newBuffer;
}

void CScriptArray::Resize(asUINT numElements)
{
    if( !CheckMaxSize(numElements) ) return;
...
void CScriptArray::RemoveRange(asUINT start, asUINT count)
{
    ...
    if (start + count > buffer->numElements) count = buffer->numElements - start;
...
void CScriptArray::Resize(int delta, asUINT at)
{
	if( delta < 0 )
	{
        if( -delta > (int)buffer->numElements ) delta = -(int)buffer->numElements;
        if( at > buffer->numElements + delta ) at = buffer->numElements + delta;
	}
	else if( delta > 0 )
	{
		// Make sure the array size isn't too large for us to handle
        ...
        if( at > buffer->numElements ) at = buffer->numElements;
	}
...
bool CScriptArray::CheckMaxSize(asUINT numElements)
{
    ...
    if( elementSize > 0 ) maxSize /= elementSize;
...
void CScriptArray::InsertAt(asUINT index, void *value)
{
    ...
    Resize(1, index); // Make room for the new element
    SetValue(index, value); // Set the value of the new element
}
...
void CScriptArray::InsertAt(asUINT index, const CScriptArray &arr)
{
	...
	if (objType != arr.objType)
	{
		...
		asIScriptContext *ctx = asGetActiveContext();
        if (ctx) ctx->SetException("Mismatching array types");
		return;
	}
...
void CScriptArray::RemoveAt(asUINT index)
{
    ...
    Resize(-1, index); // Remove the element
}
...
const void *CScriptArray::At(asUINT index) const
{
    ...
    else return buffer->data + elementSize*index;
}
...
bool CScriptArray::Equals(const void *a, const void *b, asIScriptContext *ctx, SArrayCache *cache) const
{
	if( !(subTypeId & ~asTYPEID_MASK_SEQNBR) )
	{
		...
		if( cache && cache->eqFunc )
		{
            ...
            if( r == asEXECUTION_FINISHED ) return ctx->GetReturnByte() != 0;
		...
		if( cache && cache->cmpFunc )
		{
            ...
            if( r == asEXECUTION_FINISHED ) return (int)ctx->GetReturnDWord() == 0;
...
int CScriptArray::FindByRef(asUINT startAt, void *ref) const
{
	...
	if( subTypeId & asTYPEID_OBJHANDLE )
	{
		...
		{
            if( *(void**)At(i) == ref ) return i;
		}
    }else{
		// Compare the reference directly
		for( asUINT i = startAt; i < size; i++ )
		{
            if( At(i) == ref ) return i;
		}
	}
	return -1;
}
...
{
	if ((subTypeId & asTYPEID_MASK_OBJECT) && !(subTypeId & asTYPEID_OBJHANDLE) )
        return reinterpret_cast<void*>(*(size_t*)buf); // Real address of object
    else return buf; // Primitive is just a raw data
}

// Sort ascending
void CScriptArray::SortAsc() { Sort(0, GetSize(), true); }

// Sort ascending
void CScriptArray::SortAsc(asUINT startAt, asUINT count) { Sort(startAt, count, true); }

// Sort descending
void CScriptArray::SortDesc() { Sort(0, GetSize(), false); }
...
void CScriptArray::Sort(asUINT startAt, asUINT count, bool asc)
{
	...
	if( subTypeId & ~asTYPEID_MASK_SEQNBR )
	{
        ...
        if( cmpContext == 0 ) cmpContext = objType->GetEngine()->RequestContext();
		...
		struct {
			...
			bool operator()(void *a, void *b) const
			{
				...
				if( cmpFunc )
				{
                    ...
                    if( r == asEXECUTION_FINISHED ) return (int)cmpContext->GetReturnDWord() < 0;
                }
				return false;
			}
		...
		if( cmpContext )
		{
            ...
            else objType->GetEngine()->ReturnContext(cmpContext);
		}
    }else{
...
void CScriptArray::Sort(asIScriptFunction *func, asUINT startAt, asUINT count)
{
	...
	for (asUINT i = start; i+1 < end; i++)
	{
        ...
        if( best != i ) Swap(GetArrayItemPointer(i), GetArrayItemPointer(best));
	}
...
void CScriptArray::CopyBuffer(SArrayBuffer *dst, SArrayBuffer *src)
{
	...
	if( subTypeId & asTYPEID_OBJHANDLE )
	{
		...
		if( dst->numElements > 0 && src->numElements > 0 )
		{
            ...
            else memcpy(dst->data, src->data, count*elementSize); // Primitives are copied byte for byte
		}
	}
}
...
{
    Resize(0); // Resizing to zero will release everything
}
